...
<<<<<<< HEAD
    ...
    // Terminal compatibility - "force local echo" override for SRM
    // (Send/Receive Mode, CSI 12 h/l). Every session here runs over a real
    // pty (see TerminalSession's class doc), so under normal circumstances
    // this emulator has no local keystroke-echo path at all: the pty's own
    // line discipline or the remote program already echoes typed input
    // back, the same as any other terminal-over-a-real-pty setup - see
    // TerminalEmulator.sendReceiveMode's own doc for why SRM is tracked
    // but otherwise inert. This setting is the escape hatch for the case
    // that reasoning doesn't cover: a raw/dumb connection (e.g. `nc` to a
    // bare socket, some serial links) that never echoes anything itself,
    // where typed characters would otherwise never appear on screen at
    // all. Off (default) matches every normal shell/SSH session, where
    // the host already echoes and turning this on would double every
    // character; the user has to knowingly opt in for the specific
    // sessions that actually need it.
    val FORCE_LOCAL_ECHO = booleanPreferencesKey("force_local_echo")
||||||| parent of 83c5940 (Updated and fixing:DECSCUSR cursor shape rendering Intermediate byte tracking was added to avoid confusion with the same final byte (q) with XTVERSION added and updated.OSC 8 hyperlink support added.OSC 52 clipboard support added and updated.OSC 133 support added and updated.Hyperlink custom link schemes toggle added and updated.hyperlink linkwhitelist added and updated.OSC52 onclipboardget toggle added and updated.Split screen Gridview Floatingpanel multipanelcontainer onzoomtextsize bug fixed.DCS state machine added and updated.Sixel image  decoder support added and updated.Kitty Graphics,image,terminal,keyboard protocol added and updated.Emoji/ZWJ bug fixed.OSC 4/10/11/12 color support added and updated.OSC7 current work directory support added.DECCOLM wiring support added.VT \v/FF \f->LF added and updated.SGR 8 (conceal) added and updated.SGR 53/55 (overline) added and updated.SGR 4:3 + 58/59 (undercurl + underline color) added and updated.TBC Ps 1/2 (line tab stops) added and updated.DECSLRM support added and updated.OSC 1337 (iTerm2 inline image) support added and updated.. REP — 'CSI Pn b' added and updated.CHT/CBT — 'CSI Pn I' / 'CSI Pn Z' added and updated.. XTWINOPS — 'CSI Ps t' added and updated.DECKPAM/DECKPNM — 'ESC =' / 'ESC >' added and updated.DECALN — 'ESC # 8' added and updated.DECOM added and updated.IRM added and updated.DECSCNM added and updated.LNM (Line Feed/New Line Mode, CSI 20 h/l) added and updated.SRM (Send/Receive Mode, CSI 12 h/l) added and updated.DECARM (Auto Repeat Mode, CSI ?8 h/l) added and updated.G2/G3 charset + LS2/LS3/SS2/SS3 added and updated.DECARM settings toggle added and physical keyboard support added and updated.SRM force local echo toggle added and updated.X11 Colors and preset added and updated.Show statusbar display all not closed and The problem of it staying on automatically after turning it on has been fixed.vertical scroll keyboardsettingscreen added and updated.Split screen broadcast  bug fixed.Zoom black screen bug fixed.pitch handler every tick pointercount >=2 fingers still on the screen current event snapshoting stillpinching deferioctl mid-gesture tick really deferred now this trailling commit ioctl is sending deferioctl  fixed.vertical scroll SessionSettingsScreen added and updated.Split screen broadcast  bug fixed.Zoom/resize split-crack bug fixed.Involuntary keyboard on/off after zoom (feels like pressing Enter) bug fixed.Zoom/resize black space (pinch-zoom final commit was missing) fixed.Broadcast toggle + split screen input disorder fixed.Screen tearing in Btop-like tools bug fixed.Selection handle flickering bug fixed.Zoom/font-size sync issue (on all screens) bug fixed.pinch zoom and font size 8f->4f added and updated.Sixel/Kitty images disappear when IME (keyboard) is opened/closed bug fixed.TERM UI selection updated popup new term type and none mode added.)
